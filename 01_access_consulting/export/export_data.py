...
def import_web_chat_ems():
    ...
    df1 = spark.sql("select * from itcast_dws.visit_dws")
    df1.registerTempTable('visit_dws_tmp')

    df2 = spark.read.format("jdbc").options(url="jdbc:mysql://192.168.88.161:3306/nev",
                                            driver="com.mysql.jdbc.Driver",
                                            dbtable="(select * From visit_dws) tmp",
                                            user="root",
                                            password="123456").load()

    df2.registerTempTable('visit_dws_tmp')
    df2.sql("use scrm_bi")
    df2.sql(
        "insert into scrm_bi.itcast_visit select * from visit_dws_tmp")
...
def import_web_chat_text_ems():
    ...
    df2 = spark.sql(
        "select * from web_chat_text_ems_2019_07_tmp")
